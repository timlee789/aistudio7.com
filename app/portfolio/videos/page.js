...
export default function VideoPortfolioPage() {
    ...
    const loadGalleryData = async () => {
      try {
        ...
        if (galleryResponse.ok) {
          ...
          const videoItems = galleryData.items.filter(item => {
            // Check if it's a video based on mimetype or file extension
            const isVideo = item.mimetype?.startsWith('video/') || 
                           item.path?.match(/\.(mp4|webm|ogg|mov|avi)$/i);
            
            // Also check images array for any video files
            if (item.images && Array.isArray(item.images)) {
              const hasVideos = item.images.some(img => 
                img.mimetype?.startsWith('video/') || 
                img.url?.match(/\.(mp4|webm|ogg|mov|avi)$/i)
              );
              return isVideo || hasVideos;
            }
            
            return isVideo;
          });
          
          setGalleryItems(videoItems);
        } else {
  ...
  const openModal = (item, imageIndex = 0) => {
    ...
    setModalImage({ 
      ...
      label: item.title ? item.title.toUpperCase() : 'VIDEO PORTFOLIO',
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <p style={{ 
              ...
              Experience our dynamic video content and multimedia projects
            </p>
          </div>
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px', padding: '0 20px' }}>
            <div style={{ background: 'rgba(255, 255, 255, 0.8)', borderRadius: '12px', padding: '16px', display: 'inline-block', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
              <span style={{ color: '#666', fontSize: '1rem', fontWeight: '500' }}>
                Showing {startIndex + 1}-{Math.min(endIndex, galleryItems.length)} of {galleryItems.length} video items
              </span>
              {totalPages > 1 && (
          ...
          {galleryItems.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <div style={{ background: 'rgba(255, 255, 255, 0.8)', borderRadius: '16px', padding: '40px', maxWidth: '500px', margin: '0 auto', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
                <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#333', marginBottom: '16px' }}>No Video Portfolio Items</h3>
                <p style={{ color: '#666', lineHeight: '1.6' }}>
                  We're currently updating our video portfolio. Please check back soon to see our latest multimedia work!
                </p>
              </div>
            </div>
          )}
